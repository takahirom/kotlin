...
object JvmTypesMappedToKotlinChecker : ClassifierUsageChecker {
    override fun check(targetDescriptor: ClassifierDescriptor, trace: BindingTrace, element: PsiElement) {
        val kotlinAnalogsForClass = JavaToKotlinClassMap.INSTANCE.mapPlatformClass(targetDescriptor as? ClassDescriptor ?: return)
        if (kotlinAnalogsForClass.isNotEmpty()) {
